...
def train(
    ...
    """
    print('untrained performance', (groundtruth_value.mean() - chanvese_value.mean()).item(), groundtruth_value.view(-1).max(0)[0].item(), chanvese_value.view(-1).min(0)[0].item())
    ...
    for i in range(epochs):
        ...
        for i in range(len(groundtruth_loader)):
            ...
            make chanvese image binary, and then subtract 0.5
            """
            chanvese_batch = (chanvese_batch > 0.5).float() - 0.5
            
            if flip == True:
                """
                force it to treat white as black and vica versa (randomly flip colours, and flip independently for groundtruth and chanvese)
                """
                groundtruth_batch = ((torch.rand([batchsize], device = device) > 0.5) * 2 - 1).unsqueeze(1).unsqueeze(2).unsqueeze(3) * groundtruth_batch
                chanvese_batch = ((torch.rand([batchsize], device = device) > 0.5) * 2 - 1).unsqueeze(1).unsqueeze(2).unsqueeze(3) * chanvese_batch
        ...
        this gives an indication of its performance as a classifier
        """
        print('done epoch', (groundtruth_value.mean() - chanvese_value.mean()).item(), groundtruth_value.view(-1).max(0)[0].item(), chanvese_value.view(-1).min(0)[0].item())
