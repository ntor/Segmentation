...
""" the regularisation parameter lmb CANNOT be initialised in the same way for
segmentation as for denoising """
""" the function unreg_mini in Sebastian's paper shouldn't need to be redone
here, as chanvese should already be a good starting point for reconstruction """
""" must recalculate data fitting term in pytorch, so we can compute gradients
"""
"""
THIS IS JUST A QUICK PLACEHOLDER FOR NOW

(honestly really confused by the difference between z and u in this context, so
not sure how to implement the data fitting term) """


def data_fitting(chanvese_batch, lmb1=1, lmb2=1, alpha=None, c1=None, c2=None):
    """
    chanvese_batch must a torch.tensor (ideally on gpu) of size
    ...
    # performing the reconstruction (algorithm 2), only relevant when c1, c2 are
    ...
    # performing the reconstruction (algorithm 2), only relevant when alpha is
    # calculated implicitly
    alpha = alpha.detach()

    penality_term = None  # TODO

    # [batchsize]
    return chanvese_term * chanvese_batch + alpha * penality_term


"""
ALGORITHM 2:

simultaneously perform a number of gradient descent steps on a full batch of
chanvese segmentations, or already partialy reconstructed segmentations (both
would take the argument chanvese_batch below) """
...
def reconstruct(chanvese_batch, NN, lmb, epsilon, reconstruction_steps=1):
    ...
    for i in range(reconstruction_steps):
        ...
        # detaching from previous autograd which also sets requires_grad to False
        reconstructed_batch = (reconstructed_batch - epsilon * gradients).detach()

    # send back to original device
    return reconstructed_batch.to(chanvese_batch.device)


""" a quick function for evaluating the quality of the reconstructed
segmentation according to the L2 difference between it and groundtruth """


def quality(reconstructed_batch, groundtruth_batch):
    """reconstructed_batch, and groundtruth_batch must be torch.tensors of the same
...
data in logs), which keeps reconstructing solutions until their quality (as
defined above, i.e. requiring knowledge of the ground truth) no longer keeps
decreasing """
...
def minimum(chanvese_batch, groundtruth_batch, NN, lmb, epsilon):
    ...
    while todo_mask.sum():
        ...
        minimum_batch.masked_scatter_(
            done_mask_unravel, chanvese_todo.masked_select(done_mask)
        )
        ...
        steps.index_put_(
            done_mask_unravel.nonzero(as_tuple=True),
            torch.tensor([1], dtype=torch.float, device=device),
            accumulate=True,
        )

        chanvese_todo = chanvese_todo.masked_select(done_mask.logical_not).view(
            chanvese_todo.size(0), chanvese_todo.size(1), chanvese_todo.size(2), -1
        )
        groundtruth_todo = groundtruth_todo.masked_select(done_mask.logical_not).view(
            groundtruth_todo.size(0),
            groundtruth_todo.size(1),
            groundtruth_todo.size(2),
            -1,
        )
        quality_prev = quality_new.masked_select(done_mask.logical_not)

    # outputs the final (optimal) reconstruction, their corresponding quality,
    # and the reconstruction steps required
    return minimum_batch.transpose(0, -1), final_quality, steps
