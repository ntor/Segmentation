...
class ShapeGenerator:
    ...
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.background = np.random.randint(150, 200)
        self.image = Image.new("L", (width, height), self.background)
        self._canvas = ImageDraw.Draw(self.image)

    def add_polygon(self, size=0, times=1, colour=0,nverts=10):
    ...
    def add_polygon3(self, size=0, times=1, colour=0,nverts=10):
        r = int(random.random() * (255 - colour) / 10)
        if size == 0:
            # If the user does not provide an explicit size for the polygon,
            # then take an eigth of the minimal dimensions of the image (as an
    ...
    def add_anulus(self, size=0, times=1):
        r = int(25 * random.random())
        if size == 0:
            # If the user does not provide an explicit size for the polygon,
            # then take an eigth of the minimal dimensions of the image (as an
        ...
        for i in range(times):
            ...
            # Imagedarw then draws the ellipse
            shape = [(lowerx, lowery), (upperx, uppery)]
            smallshape = [(midx/2-xr/4, midy/2-yr/4), (midx/2+xr/4, midy/2+yr/4)]
            
            self._canvas.ellipse(shape, outline=0 + r, fill=0 + r)
            self._canvas.ellipse(smallshape, outline=self.background, fill=self.background)
    ...
    def add_holes(self, numholes=40, width=2):
        for i in range(numholes):
            ...
            self.add_polygon3(size=width, times=1, colour=tint)
    ...
    def add_noise(self, sig=2):
        ...
        noise_img = random_noise(im_arr, mode="gaussian", var=0.05*sig)
